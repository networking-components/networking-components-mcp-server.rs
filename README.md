...
The exact `networking-components/networking-components-mcp-server.rs` organization, repository, service,
...
- `github_posture` — the exact networking-components repository and CI posture;
...
- `supabase_posture` — the configured networking-components Supabase health boundary;
- `neon_posture` — exact Neon organization/project and branch metadata;
- `cloudflare_posture` — the configured networking-components zone and DNS posture;
- `k8s_posture` — the exact networking-components namespace in the
  `ORESoftware/k8s-cluster` deployment plane;
- `nats_posture` — the networking-components service and dependency subjects; and
...
local stdio process accepts no bearer token. The `networking-components-mcp-http` binary
...
public resource as audience, requires the networking-components project realm, AAL2,
`mcp:read` plus `networking-components:inspect`, and a `networking-components_viewer` or
`networking-components_operator` role. It never forwards a caller token upstream. Exact
...
- Kubernetes: `ORE_MCP_K8S_ENABLED=1`, optionally
  `ORE_MCP_K8S_NAMESPACE` (otherwise the exact `networking-components` namespace); and
- NATS: `ORE_MCP_NATS_URL`, scoped to
  `networking-components.mcp.service.read.v1` and `networking-components.mcp.dependencies.read.v1`.
